...
| `POST /GetMetadataClobAndMessages` | Por cada SR consulta el array `messages`; genera un CSV de mensajes por archivo de entrada (metadata + ruta del contenido) y guarda el contenido de cada mensaje como archivo `.html` o `.txt` según su `MessageTypeCd`, en disco o en un bucket de GCP. Los campos CLOB son opcionales (`include_clob`). |
...
- [docs/ConsolidadoFinal.md](docs/ConsolidadoFinal.md) — cerrar la migración: índice maestro de búsqueda, gran total, lo no migrado y el cuadre final
...
- **Reintentos automáticos (errores transitorios)**: cada llamada al API que falle con un error transitorio — `5xx` (500/502/503/504), `429 Too Many Requests`, o errores de red/timeout — se reintenta sola con backoff exponencial (`1s, 2s, 4s, 8s…` + jitter, respetando `Retry-After` si viene). Configurable con `OSC_MAX_RETRIES` (por defecto 4) y `OSC_RETRY_BACKOFF`. Así un blip temporal del servicio no requiere ninguna acción manual.
- **Reintentos entre corridas**: si tras agotar los reintentos automáticos un SR o descarga sigue fallando, ese archivo **no** se marca como procesado, por lo que la siguiente corrida lo vuelve a tomar — en metadatos reintenta solo los SR que fallaron (gracias al checkpoint) y en binarios los archivos ya existentes en disco se omiten (`skipped_existing`), salvo que se envíe `"overwrite": true`. Basta con volver a lanzar el mismo método con los mismos parámetros.
- **Circuit breaker (servicio caído)**: si el job acumula `OSC_CIRCUIT_THRESHOLD` fallos transitorios **consecutivos** (por defecto 10 — señal de mantenimiento o caída, no de blips), se detiene solo: cancela lo pendiente sin llamar más al API, queda como `interrupted` con el motivo en el campo `error`, y el avance queda en el checkpoint. Verificar con `GET /health` que Oracle volvió y relanzar el mismo método. Un éxito reinicia el contador, así que los fallos aislados no lo disparan.
- **Paralelismo por petición**: ambos métodos aceptan `max_workers` (1–64) en el body para ajustar las llamadas en paralelo de ese job sin editar el `.env` ni reiniciar; sin él se usa `OSC_MAX_WORKERS`.
